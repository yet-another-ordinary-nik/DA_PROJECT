...
```sql
SELECT 
    DATE_TRUNC('minutes', order_date) as hour,
    COUNT(*) as orders_count,
...
После настройки Redash, добавьте скриншоты вашего дашборда в папку `screenshots/`:
- `dashboard_overview.png` - общий вид дашборда
- `sales_by_category.png` - продажи по категориям
- `orders_timeline.png` - динамика заказов
- `top_cities.png` - топ городов
...
Назаровым Николаем Кирилловичем
